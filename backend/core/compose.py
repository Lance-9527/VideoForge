...
═══════════════════════════════════════════════════════════════════
为什么需要这个模块（这是本项目此前"跑不通"的根治点）
═══════════════════════════════════════════════════════════════════
VideoForge 原有 11 个视频生成模型适配器（可灵/万相/海螺/Sora/…），
**全部需要付费 API Key**。用户一个 Key 都没配 → 「生成视频」永远失败 →
整条链路在最后一步断掉，做不出任何成品。

MoneyPrinterTurbo 之所以"开箱即用"，关键不在它的界面，而在它的**素材策略**：
它不调用视频生成大模型，而是用「图片/stock 素材 + TTS 配音 + 字幕 + FFmpeg 拼接」
合成视频。这条路**零付费 Key 就能出片**。

本模块照此思路实现 VideoForge 的自有成片路径：

    分镜文本 ─┬─ Edge TTS ──→ 语音 + 句级字幕时间轴
              └─ 画面来源 ──→ 场景图 / 角色图 / 自动生成的文字卡
                              ↓
                    FFmpeg 图片运镜(Ken Burns) + 配音 → 分段视频
                              ↓
                        拼接 → 烧字幕 → 混 BGM → final.mp4

设计原则：
- **绝不因为缺素材而失败**：没场景图就用角色图，没角色图就自动生成文字卡。
- **绝不因为没配音而失败**：可用 silent provider 出静音片段。
- **每一步都留退路**：Ken Burns 失败退化为静态图；烧字幕失败则保留 .srt 旁挂文件。
"""
...
class ClipInfo:
    ...
    trim_head: float = 0.0           # 起幅死帧裁掉了多少秒（配音轨与字幕要同步平移）
    trim_tail: float = 0.0           # 收尾定住裁掉了多少秒（只需缩短，无平移）
    stall_cut: float = 0.0           # 中间冻结段剪掉了多少秒（画面会出现一次跳切）
    audio_precut: bool = False       # 配音轨已随画面同量剪过（摆位时不要再 skip）
    removals: List[Any] = field(default_factory=list)   # 实际剪掉的区间（原始本镜时间）
...
class ComposeResult:
    ...
    # 成片级连贯度指标（全片单帧最大跳变 / 自身 p99 / 比值）。
    # ★ 必须显式声明 + ★ to_dict：不然接口里看不到，用户只能拿到一个视频文件。
    #   自己去猜"这次到底顺不顺"。目标要"可量化的接缝指标"，这一步是把它交付出去。
...
⚠ 实测：imageio-ffmpeg 的 Windows 包 **只含 ffmpeg.exe，不含 ffprobe.exe**，
而系统 PATH 里也通常没有 ffprobe。所以所有依赖 ffprobe 的地方都必须有退路，
见 probe_duration() 的 `ffmpeg -i` 解析方案。
"""
    ff = _ffmpeg()
...
async def _run(cmd: List[str], timeout: int = 900) -> Tuple[int, str]:
    """跑外部命令（统一走 core.proc：不闪窗 + 干净环境，避免 0xc0000142）。

注意：stderr 是 FFmpeg 的"正常输出渠道"（`-i` 探测、进度都打在这里），
...
async def _probe_duration_via_ffmpeg(path: str) -> float:
    """用 `ffmpeg -i` 的输出解析时长（不需要 ffprobe）"""""
...
async def probe_duration(path: str) -> float:
    """取媒体时长（秒）：优先 ffprobe，没有则退回 `ffmpeg -i` 解析。任何异常都返回 0"""
...
def make_text_card(out_path: str, title: str, body: str = "",
    ...
    # 左侧强调。
    d.rectangle([0, 0, max(6, w // 110), h], fill=(88, 166, 255))
...
⚠ 实测：shots.layer3_constraints 是 **JSON 字典**（如
{'must_not_appear': [...], 'must_keep': [...]}），
旧代码直接 `v[:120]` 会抛 `KeyError: slice(None, 120, None)` 把整条合成链路炸掉。
这里统一做类型归一。
"""
    if v is None:
        return ""
        ...
        for it in timeline:
            if isinstance(it, dict):
                for k in ("dialogue", "line", "speech", "voiceover", "narration", "subtitle"):
                    ...
                    if not v:
                        continue
                    # ★ 台词是**{character,text,emotion} 结构化**的，不是字符串。
                    #   旧代码 `str(v).strip()` 会把它变成
                    #   `{'character': '珊莎', 'text': '...'}` 这种字面量 ——
                    #   实测成片里的"旁白"就真的去念这个字典，字幕也是这么显示的。
                    if isinstance(v, dict):
...
def _pick_image(shot: Dict[str, Any], scenes: Dict[str, dict],
    ...
    """挑一张**与该分镜直接相关**的画面：分镜自带 → 场景图 → 角色图。

刻意不做"随便拿项目里任意一张图"的兜底：
实测那样会让 5 个分镜全用同一张角色图（同一画面重复 5 次），
观感比按分镜生成的文字卡更差。没有直接相关画面时交给
调用方去「自动生成画面」或「生成文字卡」。
"""
...
def image_prompt_for_shot(shot: Dict[str, Any], scenes: Dict[str, dict],
    ...
    body = "场景：".join(b for b in bits if b)
    if not body:
        body = "电影感场景"
    return (
...
async def _auto_generate_image(shot: Dict[str, Any], scenes: Dict[str, dict],
    ...
    """调用图像模型按分镜生成一张画面。返回 (路径, 错误信息)"""
    try:
        from core.imagegen import generate_image
        prompt = image_prompt_for_shot(shot, scenes, chars)
        api_key = ""
        try:
            from core.voice.base import PROVIDERS  # noqa: F401  仅为确认 voice 模块已加。
        except Exception:
            pass
...
返回实际得到的目标时长。这是"用户想要 5 秒就 5 秒"的关键：
配音文本长度不可控，但成片段长必须可控。
"""
...
async def render_shot_clip(
    ...
    *,
    ...
    """把**单个分镜**渲染成一个可播放 mp4（不需要付费视频模型 Key）。

这是「生成」页那个按钮的真正实现：借鉴 MPT 的思路 ——
用「画面（现成/AI 生成/文字卡）+ 配音 + 运镜 + 字幕」合成，
而不是去调文生视频大模型。
"""
    ...
    if not img:
        ...
        src = "text_card"
        warnings.append("该分镜无现成画面，已生成文字卡")

    # 2) 配音
    #
    # ★★ 2026-09-13 补的一刀：这条**本地渲染路径**原先与主循环不一样 ——
    #   它自己拿 `narration_for_shot()` 的结果去合成，也就是**用默认音色朗读
    #   画面概述**。主循环里的旁白已经按用户要求去掉了，这里却漏掉，
    #   于是"没有生成视频、走本地渲染"的那些分镜仍然在念稿子
    #   （实测 caa9904f 改后仍剩 2 条念稿字幕，就是这里来的）。
    #   现在两条路径统一：有台词 → 按角色逐句配；没台词 → 静音（不再念概述）。
    audio = ""
    subs: List[dict] = []
    natural = float(shot.get("duration_seconds") or 5.0)
    _lines = []
    try:
        from core.dialogue import timeline_lines as _tlx2
        _lines = _tlx2(shot.get("layer2_timeline"), natural)
    except Exception:
        _lines = []
    if with_voice and _lines:
        step(0.30, "按角色配音…")
        try:
            from core.voicecast import dub_shot as _dub2
            _d2 = await _dub2(shot, characters=list((chars or {}).values()),
                              settings=settings,
                              out_dir=os.path.join(os.path.dirname(out_path), "dub"),
                              default_voice=voice_id, duration=natural, cast=cast)
            if _d2.get("ok"):
                audio = _d2.get("path") or ""
                subs = list(_d2.get("subtitles") or [])
                natural = float(_d2.get("duration") or 0) or natural
                for _w in (_d2.get("warnings") or []):
                    warnings.append(str(_w))
            else:
                warnings.append(f"按角色配音失败，改用静音：{_d2.get('error')}")
        except Exception as e:
            warnings.append(f"按角色配音异常，改用静音：{e}")
    elif with_voice and narration and narration_fallback:
        # 显式要求旁白时才念概述（纪录片式旁白是**用户主动选择**的形态）
        step(0.30, "合成旁白…")
        try:
        ...
        warnings.append("该分镜没有台词（不做旁白朗读；要让人物说话请补台词）")
    ...
    if audio and abs(final_seconds - natural) > 0.25:
        step(0.45, f"对齐时长到 {final_seconds:.0f} 秒…")
        ...
        if os.path.exists(fitted):
            audio = fitted
    # 字幕若超出片段长度，按比例压缩到片段。
    if subs and final_seconds > 0 and subs[-1]["end"] > final_seconds + 0.1:
    ...
    if srt:
        ...
        if okb:
            ...
            warnings.append(f"字幕未烧入（已保留 srt）：{note}")
...
★★ 为什么必须有这个函数（线上真实事故）：
  `render_clip` 曾经把**视频**当**静图**喂进去（`-loop 1 -framerate 30 -i x.mp4`），
  `-loop 1` 对 mp4 并不像对 PNG 那样无限循环，于是视频轨停在源片长度（5.64s），
  而 `anullsrc` 出来的静音音轨被 `-t 10` 拉满 10 秒 →
  **容器时长 = 10.00s，视频流真实长度 = 5.64s**。
  而 `probe_duration()` 读的是容器时长，所以"想渲 10s / 实测 10s"的检查
  **完全抓不到**。等到拼接阶段丢掉音轨（`-an`）重编码，视频就"缩"回 5.64s ——
  6 段声明 60 秒的成片只剩 39.93 秒，字幕时间轴还按 60 秒排。

ffmpeg 自带的 `-i` 只报容器时长，所以这里**解码到 null** 再读最后一行 `time=`：
    ffmpeg -i x.mp4 -map 0:v:0 -f null -
这是唯一能拿到"视频轨到底有多长"的可靠办法。
"""
...
def _is_video_input(path: str) -> bool:
    """这个"素材"到底是视频还是静图 —— 两者的 ffmpeg 喂法**完全不同**。

搞错就会出现上面那个"容器 10 秒、视频轨 5.6 秒"的事故。
"""
    try:
...
async def render_clip(image: str, audio: str, duration: float, out_path: str,
    ...
    """把「素材（静图 **或** 视频片段）+ 一段音频」渲染成一个标准化的视频片段。

返回 warning 列表（空表示一切顺利）。

★ 静图与视频必须分开处理（这是踩过的坑）：
  · **静图**：`-loop 1 -framerate fps -i img` 无限供帧，可选 Ken Burns 推拉
  · **视频**：`-stream_loop -1 -i clip` 让源片**循环**着填满目标时长。
    绝不能用 `-loop 1`（对 mp4 不起作用 → 视频轨停在源片长度，
    容器时长却被静音音轨撑满 → "假的 10 秒"）；也不该套 Ken Burns
    （视频本身已经有运动，再叠推拉是双重运动，看着晕）。
    当源片**短于**目标时长时，循环填充比"留一段黑"或"截短"都好：
    用户要的是这一镜 10 秒，宁可素材循环也不要时间轴塌掉。
"""
    warnings: List[str] = []
    ...
    if is_vid:
        ...
        if src_vdur > 0.05 and src_vdur + 0.35 < duration:
            warnings.append(
                ...
                f"s —— 已循环播放补足（不会留黑，也不会把时间轴缩短）")
        elif src_vdur > duration + 0.35:
            warnings.append(
                f"素材长 {src_vdur:.1f}s，只取前 {duration:.1f}s")
        ...
        warnings.append("无可用画面，使用纯色底")

    has_audio = bool(audio) and os.path.exists(str(audio))
    if has_audio:
        a_in = ["-i", str(audio)]
    else:
        # 无配音也要有音轨，否则 concat 时各段时间轴不齐
    ...
    attempts = []
    if is_vid:
        # 视频输入：只做 scale_pad。Ken Burns（zoompan）是给静图用的，
        # 套在视频上会变成"每帧再推一下"，画面怪而且帧数会爆炸。
        attempts.append(("video", static))
    else:
    ...
    for tag, vf in attempts:
        ...
        if rc == 0 and os.path.exists(out_path) and os.path.getsize(out_path) > 1024:
            if tag == "static" and ken_burns and not is_vid:
                warnings.append("运镜失败，已退化为静态画面")
            # ★ 渲完立刻量**视频流**真实长度，容器时长不算数（见 probe_video_duration）
            vd = await probe_video_duration(out_path)
            if vd > 0.05 and vd + 0.5 < duration:
                warnings.append(
                    f"⚠ 片段视频轨只有 {vd:.1f}s，目标是 {duration:.1f}s ——"
...
两个要点（都是实测出来的）：
1. 字幕不是文章。一行超过 ~18 个汉字就糊成一片，手机上看不清；
   而"合并成更少的行"只会把行拉得更长，所以超量时**保留前几条**、
   把剩下的丢掉，而不是合并。
2. 分镜概述里混着大量**提示词残留**（"时长为10秒""镜头语言以缓慢推进为主"
   "整体色调偏冷"）—— 这些是给模型看的，不是给观众看的，当字幕很滑稽。
   这里按模式过滤掉。
"""
    ...
    for p in parts:
        ...
        for seg in _re.split(r"(?<=[，,、])", p):
            ...
            if seg:
                kept.append(seg)
    if not kept:
        # 全被过滤掉了（整段都是元信息）→ 退回不过滤的短。
        kept = [x.strip() for x in parts if x.strip()][:max_parts]
    # 超量就保留前几条（前几句通常是在交代"发生了什么"）
...
def write_srt(entries: List[dict], out_path: str) -> str:
    """写 SRT。entries: [{start, end, text}]（已含全局偏移）"""""
    lines = []
...
async def _mux_clip_audio(video: str, audio: str, out: str, duration: float) -> bool:
    """把**配音轨**替换进片段（生成视频自带的要么没有音轨、要么是静音轨）。

★★ 为什么必须单独有这一步（2026-09-13 真实事故）：
  `_normalize_clip` 只处理"源片自己的音轨 / 没有就补静音"，
  它并不知道我们为这一镜合成了配音 —— 于是 `ci.audio` **算出来了却从来没进过成片**：
  实测成片 `final.mp4` 只有 `Stream #0:0` 视频流，中间产物里的音轨是
  2 kb/s 的静音，最后响度归一化那一步干脆把音轨丢了。
  本地合成路径没这个问题 —— 它的音频是在 `render_clip` 里直接烧进片段的。
  所以"已生成视频"这条路必须补上同样一步。
"""
...
async def _cut_ranges(src: str, removals, out: str, dur: float,
    ...
    """按 `removals` 剪掉若干段，把剩下的接起来（视频或音频）。

用途只有一个：**剪掉片段中间那段真冻结**（`probe_stall_mid_windows`）。
剪中间是**动刀** —— 画面会出现一次跳切，所以只在上层显式开启时才做
...
★★ 「不够就循环补足」是这一版才补上的（线上真实事故）：
  视频模型按标称 10 秒的任务，经常只给 5.6~5.9 秒。旧代码只有
  `-t 10`（只能**截断**，不能**延长**），于是：
      视频轨 = 5.64s（源片长度）
      静音轨 = 10.00s（anullsrc 被 -t 拉满）
      容器时长 = 10.00s   ← probe_duration 读这个，所以"检查通过"
  拼接阶段一旦丢音轨重编码，视频就"缩"回 5.64s ——
  6 段声明 60 秒的成片只剩 **39.93 秒**，而字幕时间轴还按 60 秒排，
  后半段字幕全压在不存在的时间上。
  修法与 `render_clip` 一致：用 `-stream_loop -1` 真循环，
  宁可素材循环也不要时间轴塌掉。
"""
    warns: List[str] = []
    ...
    # ★ 起幅死帧：在**缩放之前**裁掉，再把 PTS 拉回 0（否则会留一段黑）。
    #   旋到 vf 的最前面是有意的：先 trim 省掉后面的缩放计算。
    if ht > 0.02:
        vf = f"trim=start={ht:.3f},setpts=PTS-STARTPTS," + vf

    # 先量**视频流**真实长度（容器时长会被静音轨撑长，不能用来判断）
    src_vdur = 0.0
    try:
        src_vdur = float(await probe_video_duration(src) or 0.0)
    except Exception:
        src_vdur = 0.0
    # 裁掉了 ht 秒之后，真正可用的素材长度要减掉它 —— 否则会以为够长、其实不够
    ...
    if need_loop:
        warns.append(
            f"生成视频只有 {src_vdur:.1f}s，要填满 {d:.1f}s —— 已循环播放补足"
            f"（不留黑、也不把时间轴缩短）")
    elif d and src_vdur > d + 0.35:
        warns.append(f"生成视频有 {src_vdur:.1f}s，按 {d:.1f}s 截断")

    # 渲完核一次**视频流**：容器时长会被静音轨撑长，只有解码到底才看得到真实长度
    out_vdur = 0.0
    try:
    ...
    if d and out_vdur > 0.05 and out_vdur + 0.5 < d:
        warns.append(
            f"⚠ 规范化后视频轨仍只有 {out_vdur:.1f}s（目标 {d:.1f}s）——"
            f"s）—— 时间轴会按实际长度排，避免字幕压到不存在的画面上")
    return warns
...
`transition`：
  - "none"  硬切（concat demuxer，最快、无损）
  - "fade"  **交叉溶解（xfade + acrossfade）—— 相邻镜头丝滑衔接**，默认
  - "auto"  同 fade，片段过短时自动退化为硬切
...
"""
    ...
    if trans in ("fade", "auto") and len(clips) >= 2 and transition_sec > 0:
        ...
        if not _valid or min(_valid) <= transition_sec * 2 + 0.2:
            logger.info("片段过短（最短 %.2fs），转场退化为硬切", min(_valid) if _valid else 0)
            trans = "none"
        else:
            inputs: List[str] = []
            for c in clips:
...
async def _burn(video: str, srt: str, out_path: str, w: int, h: int) -> Tuple[bool, str]:
    ...
    # subtitles 滤镜在 Windows 上要转义盘符冒号与反斜杠
...
★ 只写场景卡里**真实存在**的字段。一个都没有就返回空串 →
  这一处不打卡（宁可没有卡，也不要编一个不存在的地点）。
这是方法论仓库里 "提示词/画面不得创建未登记的地点"
（video-prompt-contract.md：*提示词不得创建未登记的主要角色、怪物、地点或关键道具*）
在**后期**这一侧的对应做法。
"""
    if not scene:
        return ""
    ...
    _TOD = {"day": "日", "night": "夜", "dawn": "拂晓", "dusk": "黄昏",
...
不依赖任何付费视频模型 Key —— 全程只用本地图片 + Edge TTS + FFmpeg。
"""
    ...
    step(0.02, f"准备合成：{len(shots)} 个分镜 · {w}x{h}")

    # ══════════════════════════════════════════════════════════════
    # 整片选角：一次把"每个角色用哪把嗓子"定死（2026-09-13）
    # ══════════════════════════════════════════════════════════════
    # 用户原话："配音要角色来发音……角色自己说话且有自己的音色……
    #            声音要跟人物形象符合"。
    # 旧代码是**逐分镜**挑音色（`used=set(cast_used.values())` 在分镜内局部），
    # 于是同一个角色在不同分镜里可能拿到不同的嗓子 —— 这等于"角色没有自己的声音"。
    # 这里改成整片一次定，并且**落库**（写进角色卡的 reference_features.voice_id），
    # 这样：① 重合成也稳定；② 用户在角色页能看到并手动改。
    ...
    try:
        ...
        for _c in _cp.get("collisions") or []:
            res.warnings.append(
                f"配音选角提醒：{('、'.join(_c['characters']))} 拿到了同一把嗓子"
    ...
    # 「素材短于标称」逐镜收集，最后**汇总成一条**警告 ——
    # 逐镜各发一条会把后面的重要结论（逐切点转场 / 纹理统一 / 连贯度）
    # 挤出管线结果的警告上限（`pipeline._step_base` 只取前 5 条）。
    _short_notes: List[str] = []
    _no_dialogue_shots: List[int] = []
    _narration_used = 0
    timeline_cursor = 0.0
    clip_infos: List[ClipInfo] = []
    # 每个分镜在“未做转场前的拼接轨”上的位置，用于把字幕从分镜内相对时间
    # 映射到成片绝对时间（转场会让成片比 Σdur 短，不映射就会整体错位）
    ...
    for i, shot in enumerate(shots):
        ...
        # 在「生成」页用真实视频模型（可灵/海螺/Seedance…）生成好的片段。
        # 一律从图片重新"本地合成"，于是用户花钱生成的高质量画。
        # 在“一键成片”时被整个丢掉，最后得到的还是只有字幕+AI语音的东西。
        #
        # 正确顺序：已生成的分镜视频 > 分镜/场景图 + 运镜 > 文字卡
        # ══════════════════════════════════════════════════════════
        ...
        if cand_path:
            ...
            # 时长按**素材真实长度优先，不循环重播**。
            #
            # ★★ 真实事故（2026-09-13，项目 caa9904f）：
            #   旧逻辑是"素材与标称差 >0.3s 就用标称时长"，于是 5.88s 的生成素材
            #   被 `-stream_loop -1` 拉到标称 12s —— 同一段运动**重演一次**，
            #   在成片里就是"演到一半突然跳回开头"。实测这一版成片的
            #   `spike_ratio` 高达 **6.016**（≈1 才算接缝看不出来），
            #   而全片最大跳变就来自这些**循环回跳点**，不是接缝本身。
            #   这属于"一眼AI"里最刺眼的一类：不是画质问题，是运动不连续。
            #
            # 现在的规则：
            #   · 用户**显式指定**了"每段统一时长" → 尊重他，循环补足（并如实警告）
            #   · 否则 → 按素材真实长度收尾，宁可这一镜短一点，也不重播
            ...
            if explicit_len:
                ...
                if cdur > 0.2 and cdur + 0.3 < use_dur:
                    _w = (f"这一镜指定 {use_dur:.1f}s，但生成素材只有 {cdur:.1f}s ——"
                          f"已循环补足（同一段运动会出现第二次；不想要就取消『统一时长』，让它按素材真实长度收尾）")
                    ci.warnings.append(_w)
            elif cdur > 0.2:
                use_dur = cdur
                if target_len and cdur + 0.3 < target_len:
                    _w = (f"分镜标称 {target_len:.1f}s，但生成素材只有 {cdur:.1f}s ——"
                          f"成片按**素材真实长度**收尾，不循环重播（循环会让同一段运动重演一次，看着像卡带）。要更长请把这一镜的时长调到模型能力范围内，或让它多分几段生成。")
                    ci.warnings.append(_w)
                    _short_notes.append(
                        f"第{i + 1} 镜 {cdur:.1f}s（标称 {target_len:.1f}s）")
            else:
                use_dur = target_len or 5.0
            clip = os.path.join(work, f"clip_{i:02d}.mp4")
            step(0.05 + 0.65 * i / len(shots),
                 f"使用已生成视频 {i + 1}/{len(shots)}（{cdur:.1f}s）")

            # ══════════════════════════════════════════════════════════════
            # ★★ 配音：按角色逐句念台。
            # ══════════════════════════════════════════════════════════════
            # 真实事故（2026-09-13）：走"已生成视频"的分镜**完全没有音轨** ——
            # 实测成片 `final.mp4` 只有 `Stream #0:0` 视频流，整条片子是无声的
            # （只有烧进画面的字幕）。原因不是没实现，而是
            # **`voicecast.dub_shot` 写好了却从来没被任何地方调用过**。
            #
            # 时间轴必须先按**和生成时同一套规划**重排，不能直接把草稿塞进去：
            # 草稿是按标称 12s 铺的，而画面只有 5.9s；`normalize_timeline` 对超出
            # 时长的条目是"压回最后一秒"（不是等比缩放），
            # 直接把 12s 的轴当 5.9s 用会让后面几句**叠在最后一秒里**。
            # 所以这里调 `plan_shot(audio_seconds=实测台词时长)` 拿到重排后的时间轴，
            # 它正是当初生成这一镜时用的那份。
            _dub: Dict[str, Any] = {}
            _audio_sec = 0.0
            _has_lines = False
            ...
            if with_voice:
                try:
                    ...
                    # ★ 台词要排进**画面真实的长度**（不是标称、也不是规划值）：
                    #   排不下时 `synthesize_line` 会在 MIN/MAX_SPEED 内温和变速。
                    #   并如实报溢出；**绝不能反过来把画面拉长** ——
                    #   拉长就意味着 `-stream_loop` 重播，实测那正是“卡带感”的元凶
                    #   （为让台词说完而延长画面后，spike_ratio 从 1.687 反弹到 4.034）。
                    _dub = await dub_shot(
            ...
            if _speech_end and _speech_end > use_dur + 0.35:
                ci.warnings.append(
                    f"台词说到 {_speech_end:.1f}s，画面只有 {use_dur:.1f}s ——"
                    f"结尾有 {_speech_end - use_dur:.1f}s 会被切掉。（**不再**把画面拉长来迁就台词：拉长等于让画面重播一次，看着像卡带。）彻底解决要重新生成这一镜、或把台词改短。")

            # ★★ 起幅死帧：I2V 模型常常前几帧几乎不动（从静图里"苏醒"）。
            #   换场默认改成硬切之后这段死的起幅**不再被溶解盖住** ——
            #   实测同一部片子第 5 镜起幅只有 0.26× 全片中位，观感就是"切过来卡住半秒"。
            #   治法是**裁掉死帧**，并且把这一镜的配音轨与字幕**同量平移**；
            #   否则画面提前了 h 秒、声音没提前 → 声画错位（这是不能省的第二步）。
            _sres = {}
            if not explicit_len:
                try:
            ...
            # ★ 尾巴也要管：实测另一个项目（d5815c1e）5 个镜头**全都有**结尾
            #   0.33s 定住不动 —— 模型的收尾习惯。只裁头，这些就永远留在片子里。
            #   但尾部裁切**不能切掉台词**：只有在人声已经说完之后才裁。
            #   否则宁可留着（如实说明），也不把最后一句话尾切掉。
            _tt = float((_sres or {}).get("tail_trim") or 0.0)
            if _tt > 0.02:
                if _speech_end and (_speech_end > use_dur - _tt + 0.15):
                    ci.warnings.append(
                        f"结尾有 {_tt:.2f}s 定住不动，但台词说到 {_speech_end:.1f}s，裁掉会把话尾切掉 —— 已保留。")
                    _tt = 0.0
            ...
            if _ht > 0.02 and use_dur - _ht - _tt >= 2.0:
                ...
                _w = (f"起幅几乎静止（头 0.4 秒运动只有本段中位的 "
                       f"{(_sres.get('head_settle') or 0):.2f} 倍）—— 已裁掉开头 "
                       f"{_ht:.2f}s 死帧（配音与字幕同步前移，不会声画错位）。不裁的话，硬切进来会像「卡住半秒」。")
                ci.warnings.append(_w)
            ...
            if _tt > 0.02:
                ...
                _w2 = (f"结尾有 {_tt:.2f}s 定住不动（尾部运动只有本段中位的 "
                       f"{(_sres.get('tail_settle') or 0):.2f} 倍）—— 已裁掉，"
                       f"否则观感是「演完了画面还赖着不走」。")
            #   它是内容/提示词驱动的。管线能做的只有两件：如实报出来，或者剪掉。
            #   剪的做法：画面与配音**同量剪**（否则声画错位），字幕同量平移。
            _removals: List[Any] = []
            _mid_stalls = list((_sres or {}).get("mid_stalls") or [])
            if _mid_stalls and not cut_stalls:
                # 不剪也要**告诉用户有这个选项** —— 否则他只会看到"画面停了几秒"
                # 而不知道该传什么。
                _tot = float((_sres or {}).get("mid_stall_seconds") or 0.0)
                _w_hint = (f"中间有 {_tot:.2f}s 画面几乎不动（模型没动）。这种停顿**重生成也不会消失**（实测同提示词重跑反而更长）——要么改这一镜的提示词/拆成两镜，要么传 cut_stalls=true 直接剪掉（画面会出现一次跳切）。")
                ci.warnings.append(_w_hint)
                res.warnings.append(f"分镜{i + 1}：{_w_hint}")
            if cut_stalls and not explicit_len:
                # 复用 probe_settle 已经算好的那条曲线（不再抽一次帧）
                for _w3 in _mid_stalls:
                    _removals.append((float(_w3["at"]),
                                      float(_w3["at"]) + float(_w3["seconds"])))
                if _ht > 0.02:
                    _removals.append((0.0, _ht))     # 头部死帧也一并剪。
                _dur0 = float(cdur or use_dur)
                if _removals:
                    try:
                        ...
                        _removed = 0.0
                    # 兜底：剪太多（>50%）或太少（<0.5s）都不做，别把片子剪碎。
                    if 0.5 <= _removed < _dur0 * 0.5:
                        ...
                        if await _cut_ranges(cand_path, _removals, _cp, _dur0):
                            ...
                            _w4 = (f"中间有 {_removed:.2f}s 画面几乎不动（模型没动）—— 已**剪掉**，画面在那里是一次跳切。这比让画面停住自然，但确实是动刀。")
            ...
            if cand_path:
                ...
                # ★ 字幕：必须走统一的台词解析。
                #   `dialogue` 现在是 `{character,text,emotion}` **结构化**的。
                #   旧代码 `(it.get("dialogue") or "").strip()` 会对 dict 调 .strip()
                #   → AttributeError，直接把整条合成流程搞崩。
                #   这里存的是**分镜内相对时间**，拼接完成后再用
                #   continuity.final_timeline 映射到成片绝对时。
                #   （转场会吃掉时长，不映射就会整体错位）。
                from core.dialogue import timeline_lines as _tl_lines2
                added = 0
                # ★ 画面裁过头的 `_ht` 秒、以及中间剪掉的 `ci.removals` 段，
                #   字幕时间轴都必须**同量平移**，否则从这一镜起字幕会比画面晚。
                #   头部是均匀平移，中间剪切是**分段**平移 —— 用统一映射函数算。
                _rem: List[Any] = list(ci.removals or [])
                if not _rem and _ht > 0.02:
                    _rem = [(0.0, float(_ht))]
                # 草稿时间轴的窗口：成片时间 + 被剪掉的总量（否则后面几句会被夹掉）
                ...
                if _dub.get("ok") and (_dub.get("subtitles") or []):
                    ...
                    for ln in _tl_lines2(shot.get("layer2_timeline"), use_dur + _shift):
                        ...
                        added += 1
                    # 没有台词的分镜：把画面概述**切成短句**分条显示。
                    # 旧做法是把整段概述（常常 100+ 字）当一条字幕糊满整个镜头 ——
                    # 屏幕上糊一大片字、10 秒不换，比没有字幕还难看。
                    # ★ 但这只是**估算**（按句子平均铺）。真正准的时间在下面 TTS 的
                    #   返回值里（`_r.subtitles` 是词级时间）—— 实测过"估算"的偏差：
                    #   同一个镜头开头偏 +1.65s、中间偏 −0.74s，而 TTS 真实时间
                    #   能把偏差压到 0.1s 量级。所以 TTS 成功后要**替换**掉这里的估算。
                    _est_mark = len(srt_entries)
                    # ★★ 这里也必须跟着 `narration_fallback` 走。
                    #   实测漏掉了这半边的后果：没有台词的分镜**画面上还挂着一行
                    #   剧情概述字幕**（按句子平均铺的时间），但**没有对应的声音** ——
                    #   也就是"屏幕上写着没人说的话"。用户要的是"角色自己说话"，
                    #   所以默认模式下这里一条概述字幕都不该出现。
                    if not added and ci.narration and narration_fallback:
                        ...
                        for k, ck_ in enumerate(chunks):
                            ...
                            added += 1
                    # 没有台词时也要有声音：用默认音色念画面概。
                    # （与本地合成路径一致；否则这一镜就是静音，一对比就"断"）
                    #
                    # ★★ 2026-09-13 用户裁定：**这条默认关掉了**。
                    #    用户原话："配音要角色来发音而不是从到到尾念稿子"。
                    #    方法论依据（`Hell-Grind-AIGC-Skill / failure-diagnosis.md`）：
                    #    `F-AUDIO-POLLUTION` = "自动音乐、**旁白**、字幕或无关声"。
                    #    也就是"在用户没要求旁白时自动加旁白"本身就是一类**明列的失败**。
                    #    而且这条旁白念的是 `layer1_overview`（剧情概述），
                    #    正是 `F-DIALOGUE-VISUALIZED` 想避免的"把叙述当台词"。
                    #
                    #    正确做法：没有台词 → **这一镜就是没有对白**（静音/只剩环境与音乐），
                    #    并在告警里明确指出"要让角色说话，去补台词"。
                    #    想要旧行为可以显式传 `narration_fallback=True`（保留给
                    #    "纪录片式旁白"这种**用户主动选择**的形态）。
                    if with_voice and not ci.audio and ci.narration and narration_fallback:
                        try:
                            from core.voice.base import TTSRequest
                            from core.voice import dispatcher as _vd
                            # ★ 旁白文本必须先按**画面长度**截短：
                            #   实测一个 20.25s 的镜头，概述念完要 **28.8s**，
                            #   结果结尾 8.5s 的话全被切掉（用户听感就是"话没说完"）。
                            #   4.3 字/秒是略保守的中文语速估计，留 0.6s 收尾。
                            _avail = max(16, int((use_dur - 0.6) * 4.3))
                            _txt = ci.narration
                            _trimmed = 0
                            if len(_txt) > _avail:
                                _cut = _txt[:_avail]
                            ...
                            if _r.success and _r.audio_path and os.path.exists(_r.audio_path):
                                ...
                                if _trimmed:
                                    ci.warnings.append(
                                        f"旁白按画面长度截短（去掉 {_trimmed} 字），否则会超出这一镜。")
                                if _nd > use_dur + 0.35:
                                    ci.warnings.append(
                                        f"旁白仍然比画面长（{_nd:.1f}s > {use_dur:.1f}s），结尾会被切掉一点。")
                        except Exception as e:
        ...
        # 画面：先用现成的（分镜图/场景图/角色图）；没有就按分镜自动生成，再没有才用文字卡
        img, src = _pick_image(shot, scenes, chars)
        if not img and auto_images:
        ...
        if not img:
            ...
            src = "text_card"
            ci.warnings.append("该分镜无可用画面，已自动生成文字卡")
        ci.image, ci.image_source = img, src

        # 配音：**只念真台词**，不再默认朗读画面概述 —— 见上面的长篇说明。
        audio_path = ""
        subs: List[dict] = []
        ...
        if with_voice and _lines_here:
            ...
            # ★ 这一镜**没有台词**。旧行为是拿默认音色把画面概述念一遍，
            #   用户的原话是"从头到尾念稿子，很恶心"。现在如实静音并告诉他怎么办。
            _no_dialogue_shots.append(i + 1)
        ...
        # 字幕：TTS 给的是**分镜内相对时间**，这里存相对时间，
        # 拼接完统一映射到成片绝对时间（转场会吃掉时长，不映射必错位）
        for s in subs:
        ...
        # ══════════════════════════════════════════════════════════════
        # 踩到的真事（和第十五章那个"假时间"同一族）。
        #   分镜标称 10 秒，但模型只生成了 5.88 秒的视频。
        #   `render_clip(..., 10, ...)` 变不出那 4.12 秒，产物只有 5.88 秒，
        #   而 `ci.duration` 仍然写着 10.0。
        #   后果：`timeline_cursor`/`shot_bounds` 按 60 秒排，
        #   字幕映射出来的成片长度是 58.95s，**而真正的成片只有 39.93s**
        #   —— 后半段字幕全压在不存在的时间上。
        #   实测 6 个片段声明 60.0s、成片 39.93s，差的 20 秒就是这里来的。
        #   所以：渲完立刻 probe，把**实际**时长写回去，后面一律用它。
        real_dur = 0.0
        try:
            # ★ 必须量**视频流**长度，不能用容器时长。
            #   容器时长会被静音音轨撑长（实测 clip_00：容器 10.00s / 视频流 5.64s），
            #   拿容器时长来对账等于"用一个假的数去校验另一个假的数"。
            real_dur = float(await probe_video_duration(clip) or 0.0)
            if real_dur <= 0.2:
        ...
        if real_dur > 0.2 and abs(real_dur - ci.duration) > 0.35:
            res.warnings.append(
                f"分镜{i + 1}：想要 {ci.duration:.1f}s，实际只有 {real_dur:.1f}s（素材本身就没那么长）—— 时间轴已按**实际时长**排，避免字幕压到不存在的画面上。")
            ci.warnings.append(
                f"计划 {ci.duration:.1f}s，实际 {real_dur:.1f}s（按实际记录）")
        ...
        step(0.05 + 0.65 * (i + 1) / len(shots),
             f"已渲染 {i + 1}/{len(shots)} 个分镜（{ci.duration:.1f}s · {src}）")

    # 汇总"素材短于标称"这一条（逐镜详情在各 clip 的 warnings 里，这里只给一条总账）
    if _short_notes:
        res.warnings.append(
            f"有 {len(_short_notes)} 个分镜的生成素材**短于标称时长**，成片按素材真实长度收尾、不循环重播："
            + "；".join(["收尾、不循环重播"] + _short_notes)
            + "。要更长请把这些分镜的时长调到模型能力范围内，或让它多分几段生成。")
    res.clip_count = len(clip_paths)
    res.clips = [ci.__dict__ for ci in clip_infos]

    # ── 2. 拼接：**逐切点**选转场（不再一律 fade）──
    #
    # 为什么不再一律溶解：真实影片里**同一场戏的镜头之间几乎都是硬切**。
    # 处处溶解反而一眼假（用户反馈的"情景转化衔接一眼AI"里有它一份）。
    # 规则：同场景内 → 硬切；跨场景 → 短溶解；时间/地点大跳 → 长溶解。
    step(0.75, "按镜头关系选择转场并拼接…")
    merged = os.path.join(work, "merged.mp4")
    cplan: List[Dict[str, Any]] = []
    # 实际生效的转场计划（`assemble` 的自适应摊薄会改转场时长）
    # ★ `concat_with_transitions` 的 `cuts` 回灌；没有它时间轴会整体偏）
    cplan_actual: List[Dict[str, Any]] = []
    n_hard_cuts = 0        # 成片里"换场硬切"的处数（给成片连贯度指标做语境说明）
    try:
    ...
    try:
        if cplan:
            try:
                ...
                # ★ 把每个镜头的场景 id 传下去：接缝处理只在**同一场景内**做调色匹配
                #   （白天和夜晚本来就该不一样，跨场景强行拉平反而假）。
                _scene_ids = [str(s.get("scene_id") or "") for s in shots[:len(clip_paths)]]
                # ★ 地点字幕卡文本（换场时打在黑场上）：从场景卡里取
                #   "地点 · 内/外景 · 时段"。场景卡缺字段就退化成场景名，
                #   一个字段都没有就不打卡（不许编地点）。
                _card_texts = [_scene_card_text(scenes.get(_sid)) for _sid in _scene_ids]
                ...
                # ★★ 实际生效的转场时长（`cuts`）**必须回灌**给时间轴映射。
                #   真实事故（2026-09-13，项目 d5815c1e）：片段总长 25.68s、
                #   成片只有 **21.58s**，差 4.1s —— 因为 `assemble` 里的
                #   **自适应摊薄层**把 4 处硬切改成了 0.9s 溶解，
                #   而字幕映射与配音摆放用的是 `cplan` 里"硬切 = 0 秒"的版本。
                #   结果声音与字幕**整体比画面晚约 4 秒**：
                #   末几条字幕落在 22.2～24.5s，而片子 21.58s 就结束了 ——
                #   实测声画同步 9/17，字幕压在空气中。
                _cuts_actual = [float(c or 0.0) for c in (cres.get("cuts") or [])]
                cplan_actual = []
                ...
                res.warnings.append(
                    f"逐切点转场：{len(clip_paths)} 个镜头 · "
                    f"{len(cuts)} 处溶解 / {len(clip_paths) - 1 - len(cuts)} 处硬切"
                    + (f" · 转场合计削减 {sum(cuts):.2f}s" if cuts else ""))
                # ★★ 换场处理：哪些跨场景接缝被判定"明暗突变"、上了黑场+地点卡。
                #   必须报出来 —— 用户点名的就是"突然特别亮/特别暗"，
                #   如果系统悄悄处理了却不说，他既看不到效果也没法复核。
                _hm0 = cres.get("harmonize") or {}
                for _st in (_hm0.get("scene_treatment") or []):
                    if _st.get("treatment") == "dip":
                        ...
                        res.warnings.append(
                            f"换场 {_st.get('seam')}：{_st.get('why')}")
                # ★ 把接缝处理的结论如实带出来：画幅是否统一过、裁了多少、
                #   接缝强度从多少降到多少、哪些切点被"摊薄"了。
                #   用户要能看到"为什么这次更顺了"——不然改了也等于没改。
                _hm = cres.get("harmonize") or {}
                if _hm.get("applied"):
                    _t = _hm.get("target") or {}
                    _bt = (_hm.get("before") or {}).get("mean_seam_ratio")
                    _at = (_hm.get("after") or {}).get("mean_seam_ratio")
                    res.warnings.append(
                        f"接缝预处理：画幅已统一到 {_t.get('w')}x{_t.get('h')}"
                        f" @ {_t.get('fps'):g}fps"
                        + (f"，接缝强度 {_bt} → {_at}（越接近 1 越自然）"
                           if _bt is not None and _at is not None else ""))
                # ★★ 2026-09-13 修：`harmonize` 的**发现**（notes/warnings）以前写在
                #   `if _hm.get("applied"):` **里面** —— 于是只要"画幅本来就一致、
                #   不需要归一化"，它报的所有东西都被**静默丢掉**。
                #   实测：新加的"多个分镜画面几乎完全相同"告警因此完全没出现
                #   （项目 46c75ddf 的 5 个镜头是同一张肖像，10 对相似度 0.935~0.957）。
                #   告警讲的是**素材本身的事实**，跟"这次有没有动手改"无关，
                #   所以必须无条件上报。（同理，"画幅统一…"那条说明以前也一起丢了。）
                for _wn in (_hm.get("warnings") or []):
                    res.warnings.append("接缝：" + str(_wn))
                for _n in (_hm.get("notes") or []):
                    res.warnings.append("接缝：" + str(_n))
                # ★ 转场摊薄的结论**不管有没有归一化都要报** ——
                #   画幅本来就统一时 `applied` 是 False，但自适应转场照样在起作用
                #   （实测线上有的项目画幅统一、却全用硬切，接缝照样跳）。
                for _s in (_hm.get("softened") or []):
                    res.warnings.append(
                        f"接缝 {_s.get('seam')}：实测跳变是自身运动尺度的 "
                        f"{_s.get('seam_ratio')} 倍，已改用 {_s.get('seconds')}s "
                        f"溶解摊薄（硬切会明显跳）")
                if _hm.get("respected_cuts"):
                    res.warnings.append(
                        f"按设置保留了 {len(_hm['respected_cuts'])} 处硬切"
                        f"（未做自适应摊薄）—— 这些切点实测跳变较大，"
                        f"但「换场硬切」本身就是成立的电影语法，"
                        f"而且硬切不存在「两张画面叠在一起」的问题。"
                        f"想要溶解就传 scene_transition=dissolve。")
                #   沉默会被读成"量过、很自然"。这里的分母是"片段自身的运动尺度"。
                #   图片卡/纯静止素材根本没有这个尺度（见 `seamfix.motion_base`）。
                for _u in (_hm.get("unmeasurable") or []):
                    res.warnings.append(
                        f"接缝 {_u.get('seam')} 的强度**没量出来**：{_u.get('why')}"
                        f"因此这一处按镜头关系处理，而不是按某个实测倍数。（注意：这不等于「这处接缝很自然」，只是没有参照物。）")
                # ★ 片段**内部**的停顿：接缝看不出来 ≠ 片子里没有卡住的地方。
                #   实测第 1 镜内部有 2.88 秒几乎静止 —— 这一条以前没有任何告警。
                _st = _hm.get("stall") or {}
                ...
                if _fz:
                    ...
                    for _k, _v in sorted(_by.items(), key=lambda kv: str(kv[0])):
                        # ★ 必须**逐窗口**如实描述：早先这里写的是
                        #   "最大 {max(seconds)} @ {min(at)}"，把两个不同窗口的数
                        #   拼成了一句 —— 实测那句变成"最大 2.9s @ t=0.1s"。
                        #   而 2.9s 其实在 t=7.3s。假数字比不说更糟。
                        _mid = [x for x in _v if 0.6 < float(x.get("at") or 0)
                        ...
                        if _mid:
                            ...
                            _bits.append(f"片中 t={_w.get('at')}s 起 "
                        ...
                        if _tail:
                            ...
                            _bits.append(f"片尾定住 {_w3.get('seconds')}s"
                                         f"（t={_w3.get('at')}s 起）")
                        _parts.append(f"第{_k} 镜：" + "、".join(_bits))
                    ...
                    res.warnings.append(
                        f"⚠ 片段内部停顿：{len(_fz)} 处「几乎不动」的段落（其中 {_n_mid} 处"
                        f"在片段中间）—— " + "、".join(_parts) + "。这**不是**接缝问题（接缝可能完全看不出来），是模型生成时自己卡住了，观众的感受是「画面停了几秒」。片中卡住建议重新生成该镜、或把长镜头拆成两镜；片头偏静若紧接在转场之后，多数已被溶解盖住。")
                # ★ 纹理统一的结果也要报：用户要能看到"为什么这次更像一个片子"。
                _tx = _hm.get("texture") or {}
                _rb, _ra = _tx.get("ratio_before"), _tx.get("ratio_after")
                if _rb is not None and _ra is not None and _rb > 1.15:
                    res.warnings.append(
                        f"纹理统一：各段高频纹理能量相差 {_rb:.2f} → "
    ...
    # ★ 字幕时间轴映射：转场让成片比 Σdur 少 (n−1)×t 秒。
    #   不做这一步，从第二个镜头起所有字幕都会整体提前、越往后偏越多
    #   —— 这正是"语音字幕和画面对不上"的机制之一。
    if cplan and srt_entries:
        try:
            from core.continuity import final_timeline as _ftl
            real_durs = [await probe_duration(c) or 0 for c in clip_paths]
            # ★ 用 `cplan_actual`（实际生效的转场时长），不是 `cplan`。
            #   自适应摊薄会把硬切改成溶解，用旧计划会让字幕整体偏后（见上面的注释）。
            tl = _ftl(shots[:len(clip_paths)], cplan_actual or cplan, real_durs)
            ...
            for e in srt_entries:
                ...
                if not b:
                    continue
                x = dict(e)
                # ★ 关键：end 必须夹到**该段真实片长**之内。
                #   分镜标称 10s、实际片段只有 5.6s 时，不夹的话字幕会
                #   一直显示到下一个镜头里去（实测字幕 0～10s 盖住了第 2 个镜头）。
                limit = b["local_duration"]
            ...
            if remapped:
                ...
                if tl["timeline_shrink"] > 0.01:
                    res.warnings.append(
                        f"字幕已按转场补偿映射到成片时间轴（成片 {tl['total']}s，转场吃掉 {tl['timeline_shrink']}s）")
        except Exception as e:
    ...
    current = merged
    if srt_entries:
        # 用统一的字幕模块写 SRT：它会把每条 end 夹到下一句起点之前。
        # 并保证最短显示时长（不夹的话两句字幕会重叠、屏幕上一起闪）。
        try:
        ...
        if burn_subtitles:
            ...
            res.warnings.append("按设置跳过字幕烧录，仅生成 subtitles.srt")
    else:
        res.warnings.append("没有生成任何字幕（可能未启用配音）")

    # ══════════════════════════════════════════════════════════════
    # ★★ 配音轨：按**成片绝对时间**把每一镜的音轨摆好，混成一条。
    # ══════════════════════════════════════════════════════════════
    # 为什么必须这样做（而不是把音轨烧进每个片段）：
    #   `assemble.normalize_picture` **明确 `-an` 丢掉音轨** —— 中间产物一律无音轨
    #   （避免素材原声串味、避免 concat 流不一致），设计上音频就是"最后一次性挂上、
    #   共同原点 t=0 → 天然同步"。所以片段自带的音轨**活不到成片**。
    #   必须在这里按绝对时间重建一条。
    #   而在此之前 compose 从来没建过这条轨：`mux_film` 两次调用传的都是
    #   `audio=""`，于是成片实测只有一条视频流 —— **整条片子是无声的**。
    #   位置用 `continuity.final_timeline`：它和字幕映射用的是同一套（转场吃掉的
    #   时长会被算进去），否则声音与画面会从第二个镜头起越差越多。
    voice_track = ""
    ...
    if with_voice:
        ...
        if cplan:
            try:
                ...
                logger.warning("配音轨时间映射失败（按 0 起点排列）：%s", e)
        placements = []
        _cursor = 0.0
        for ci in clip_infos:
            ...
            placements.append({"path": ci.audio, "start": round(st, 3),
                               ...
                               # 画面裁掉 h 秒死帧 → 声音同量前移，否则这一镜声画错位。
                               # 但若配音轨**已经随画面剪过**（`audio_precut`）
                               # 即中间冻结段那次剪切），就不能再 skip：那会重复平移。
                               "skip": (0.0 if getattr(ci, "audio_precut", False)
        ...
        if placements:
            try:
                ...
                if _tr.get("ok"):
                    ...
                    res.warnings.append(
                        f"配音已挂上：{len(placements)} 个分镜的音轨按成片时间轴混成一条"
                ...
                logger.exception("配音轨混音失败")
                res.warnings.append(f"配音轨混音失败：{e}")
        elif with_voice:
            res.warnings.append("没有可用的配音音轨（各镜都没合成出配音）")

    # ── 4. 声音：BGM 避让人声 + 整体响度归一化 ──
    #
    # 旧实现直接用 `mix_bgm`（amix 相加）：既没有避让（人一说话音乐糊上）
    # 压住对白），也没有响度归一化（不同 TTS 厂商电平差近 10dB。
    # 一段响一段轻）。用户说的"特别不协调"这两条都占。
    dur_now = await probe_duration(current) or 0
    if bgm_path and os.path.exists(bgm_path):
        ...
        try:
            ...
            if mr.get("ok") and os.path.exists(mixed) and os.path.getsize(mixed) > 1024:
                current = mixed
                if mr.get("ducked"):
                    res.warnings.append("背景音乐已做「人声优先」自动避让 + 响度归一化")
            else:
                res.warnings.append("BGM 混流未产出文件，已跳过")
        except Exception as e:
    ...
    # ══════════════════════════════════════════════════════════════
    # ★★ 最后再量一次**成片本身**：全片有没有"某一帧突然跳一下"。
    # ══════════════════════════════════════════════════════════════
    # 前面所有的接缝处理（归一化 / 调色 / 转场摊薄 / 纹理统一 / 裁重复头）
    # 最终都要落到“这条成片看起来连不连贯”上。这里给出那个数字，
    # 用户在成片结果里就能看到，而不是只有一个视频文件让他自己猜。
    try:
        from core import seamfix as _sf
        fm = await _sf.film_report(final)
        res.film_metrics = fm
        if fm.get("ok"):
            # ★ 语境很重要：**换场硬切会被算进「全片最猛的一帧」**。
            #   实测同一部片子：4 处硬切 → spike 10.65；改 0.9s 溶解 → spike 1.45。
            #   只报数字会让人以为“硬切版是坏的”，而换场硬切其实是最基本的
            #   电影语法 —— 指标只是看不出"换场"这件事。所以有硬切时必须说清楚。
            _extra = ""
            if n_hard_cuts:
                _extra = (f"（本片有 {n_hard_cuts} 处**换场硬切**：它们会被计入「全片最大跳变」—— 那是「切一刀」，不是接缝缺陷。想改用溶解就传 scene_transition=dissolve）")
            res.warnings.append(
                f"成片连贯度：全片单帧最大跳变 {fm['max_delta']}，"
                f"自身常态上限 {fm['p99_delta']}，比值 {fm['spike_ratio']}"
